...
template Battleship() {
    ...
    // hash check
    component sha256p = Sha256packed();
    sha256p.inp <== carrierX + carrierY * 16 + carrierO * (16**2) + battleshipX * (16**3) + battleshipY * (16**4) + battleshipO * (16**5) + cruiserX * (16**6) + cruiserY * (16**7) + cruiserO * (16**8) + submarineX * (16**9) + submarineY * (16**10) + submarineO * (16**11) + destroyerX * (16**12) + destroyerY * (16**13) + destroyerO * (16**14);

    shipHash[0] === sha256p.out[0];
    shipHash[1] === sha256p.out[1];
